...
export async function downloadQuotePDF(quote: any): Promise<void> {
  ...
  autoTable(doc, {
    startY: y,
    head: [['Prestation', 'Description', 'Qté', 'Prix unitaire', 'Total']],
    body: tableBody,
    theme: 'grid',
  ...
  doc.setFontSize(12)
  doc.setFont('helvetica', 'bold')
  doc.text(`Total HT : ${formatEUR(quote.totalHT)}`, 182, y, { align: 'right' })
...
export async function downloadInvoicePDF(invoice: any): Promise<void> {
  ...
  doc.setFontSize(12)
  doc.setFont('helvetica', 'bold')
  doc.text(`Total HT : ${formatEUR(invoice.totalHT)}`, 182, y, { align: 'right' })
